...
# BASE_HOME_OFFSET_WORLD_DEG = 111.0
...
# RED_BIN_WORLD_DEG = -80
# BLUE_BIN_WORLD_DEG = 80
...
#     # Preserve the motor controller's normal limits.
#     old_speed, old_acceleration, old_actuation = (
#         gripper.control.limits()
#     )

#     try:
#         # Same purpose as run_until_stalled(..., duty_limit=...):
#         # restrict available motor actuation/torque.
#         gripper.control.limits(
#             old_speed,
#             old_acceleration,
#             duty_limit,
...
#         # Equivalent intention to then=Stop.HOLD.
#         gripper.hold()
...
#     finally:
#         # Restore the normal controller limits.
#         gripper.control.limits(
#             old_speed,
#             old_acceleration,
#             old_actuation,
...
def grip_attempt(speed, duty_limit, timeout_ms=5000):
    ...
    # Preserve the motor controller's normal limits.
    old_speed, old_acceleration, old_actuation = gripper.control.limits()

    try:
        # Limit available actuation similarly to run_until_stalled(...,
        # duty_limit=...), while keeping this loop non-blocking so telemetry
        # continues at TELEMETRY_PERIOD_MS.
        gripper.control.limits(
            old_speed,
            old_acceleration,
            duty_limit,
        ...
        # Hold the grasp at the detected stall position.
        gripper.hold()
        completed = True
        ...
        # If anything failed before a stable hold was reached, make sure
        # the motor cannot continue driving.
        if not completed:
            gripper.brake()

        # Restore the controller's normal limits for subsequent motions.
        gripper.control.limits(
            old_speed,
            old_acceleration,
            old_actuation,
...
def main():
    ...
    try:
        ...
        # The physical task is complete; disconnect immediately so the
        # coordinator reports PHYSICAL_DISCONNECTED instead of stale data
        # while the final speech message is playing.
        close_socket()
