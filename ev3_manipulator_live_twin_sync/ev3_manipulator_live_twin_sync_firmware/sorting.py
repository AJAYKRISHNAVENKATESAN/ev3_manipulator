...
VALID_COLORS = (   # Defining the colors as a tuple using the custom enum class -> Color. Anything can be used that is relevant to the code 
    Color.RED,
    Color.BLUE,
    Color.BLACK,
    Color.GREEN,
...
COLOR_MAP = {   # Dictionary to store the color value pairs -> config file that is referenced throughout the code. If anything has to be changed later, we make the changes here, suppose we crimson instead of red , we "crimson" with "red"
    Color.RED: "red",
    Color.BLUE: "blue",
    Color.BLACK: "black",
    Color.GREEN: "green",
...
GRIP_CLOSE_SPEED = 150
GRIP_CLOSE_DUTY_LIMIT = 40
...
# Human-facing pacing only.
BALL_COLOR_CONFIRM_MS = 500
BALL_ACTION_DELAY_MS = 1000
BALL_PROMPT_SETTLE_MS = 250
...
def announce(text):
    """Speak operator-facing prompts and mirror them to the console."""
    print("[ANNOUNCE] {}".format(text))
...
def wait_for_motor_done(motor, timeout_ms, label):
    ...
    while not motor.control.done():
        ...
        if timer.time() > timeout_ms:
            timeout_angle = motor.angle()
            timeout_speed = motor.speed()
            motor.brake()
            raise RuntimeError(
                "{} motion timeout: angle={:.1f} deg, speed={:.1f} deg/s".format(
                    label,
                    timeout_angle,
                    timeout_speed,
                )
...
def grip_attempt(speed, duty_limit, timeout_ms=5000):
    """Close gripper until stalled while continuing twin telemetry."""

    start_angle = gripper.angle()
    timer = StopWatch()

    old_speed, old_acceleration, old_actuation = (
        gripper.control.limits()
    )

    # Pybricks requires the controller to be stopped before changing limits.
    gripper.stop()
    gripper.control.limits(
        old_speed,
        old_acceleration,
    ...
    try:
        ...
        while not gripper.control.stalled():
            publish_state()

            if timer.time() > timeout_ms:
                raise RuntimeError("Gripper stall timeout.")

            wait(TELEMETRY_PERIOD_MS)

        stall_angle = gripper.angle()

        print(
            "[GRIP_CAL] start={:.1f} stall={:.1f} travel={:.1f}".format(
                start_angle,
                stall_angle,
                stall_angle - start_angle,
            )
        )
    ...
    # Restore normal controller settings before re-enabling HOLD.
    gripper.stop()
    gripper.control.limits(
        old_speed,
        old_acceleration,
...
def release_ball():
    """Return the physical gripper to the same encoder-zero open pose.

    initialize_gripper() defines the startup open pose as 0 deg. Returning to
    that absolute encoder target prevents the gripper reference from drifting
    from cycle to cycle (for example 0 -> 52 -> -39 -> 84).
    """
    run_target_streamed(
        gripper,
        abs(GRIP_RELEASE_SPEED),
        0.0,
        "gripper release",
    )
    wait_with_telemetry(GRIP_SETTLE_MS)
    print("[GRIP] opened, angle={:.1f} deg".format(gripper.angle()))
    publish_state(force=True)
...
def wait_for_ball_color():
    """Require a valid color to remain stable before accepting the ball."""
    global current_color_name

    while True:
        publish_state()
        candidate = color_sensor.color()

        if candidate in VALID_COLORS:
            confirm = StopWatch()
            stable = True

            while confirm.time() < BALL_COLOR_CONFIRM_MS:
                publish_state()

                if color_sensor.color() != candidate:
                    stable = False
                    break

                wait(TELEMETRY_PERIOD_MS)

            if stable and color_sensor.color() == candidate:
                current_color_name = COLOR_MAP[candidate]
                publish_state(force=True)
                return candidate
...
def conveyor_black():
    start_angle = conveyor.angle()
    print(
        "[CONVEYOR_BLACK] start angle={:.1f} deg speed={:.1f} deg/s".format(
            start_angle,
            conveyor.speed(),
        )
    )
    ...
    end_angle = conveyor.angle()
    print(
        "[CONVEYOR_BLACK] done angle={:.1f} deg travel={:.1f} deg".format(
            end_angle,
...
def execute_ball_cycle(cycle_id):
    ...
    announce("Place a ball")
    wait_with_telemetry(BALL_PROMPT_SETTLE_MS)

    color = wait_for_ball_color()
    color_name = COLOR_MAP[color]

    announce(color_name + " ball")
    print("[BALL] cycle={} color={}".format(cycle_id, color_name))

    # Deliberate visible pause before conveyor motion starts.
    wait_with_telemetry(BALL_ACTION_DELAY_MS)
